import os
def species(threads,pair_1,pair_2,database,output):
        if not os.path.exists(output + "/_MIST_species_alignment/"):
                os.mkdir(output+ "/_MIST_species_alignment/")
        index_list=os.listdir(database)
        for i in index_list:
        key=[];Seq={}
        alignment_filePath=output + "/_MIST_species_alignment/"
        files = os.listdir(alignment_filePath)  # 得到文件夹下的所有文件名称
        for file in files:  # 遍历文件
                seq={}
                if not os.path.isdir(file):  # 判断是否是文件夹，不是文件夹才打开
                        f = open(alignment_filePath + "/" + file, 'r');  # 打开文件
                        value = [];number=0
                        for line in f:# f.decode("utf8","ignore")
                                li = line.strip()
                                if not li.startswith("@"):  # 跳过@前三行
                                        a = li.split()  # 以tab键划分   
                                        b = a[2:3]; c=b[0];
                                        d=a[9:11];e=a[0:2];
                                        key.append(c)
                                        d=(d[0]+" "+d[1]).split(",")
                                        e=(e[0]+"_"+e[1]).split()
                                        f=dict(zip(e,d))
                                        #print(f)
                                        if c not in seq:
                                                seq[c]=f
                                        else:
                                                seq[c]={**seq[c],**f}
                #       print(seq)
                        for k,v in seq.items():
                        for k,v in seq.items():
                                if k!="*":
                                        #print(k)
                                        fw= open("_MIST."+k+".fq", 'w')
                                        for i,j in seq[k].items():
                                                #print(j)
                                                #print(type(j))
                                                m=j.split( )
                                                fw.write("@"+i+"\n")
                                                fw.write(m[0]+"\n")
                                                fw.write("+"+"\n")
                                                fw.write(m[1])
                                                fw.write("\n")
                                        fw.close()


        DICT = dict.fromkeys(key, 0)
        for i in key:
                DICT[i] += 1
        DICT.pop("*")
#       print(seq)
        import pandas as pd
        data=pd.DataFrame(list(DICT.items()),columns=['species','read_count'])
        #data=pd.DataFrame.from_dict(DICT,orient="index",columns=["read_count"])
        #data=data.reset_index().rename(columns={'index':'species'})
        print(data)
        data.to_csv(output+"_MIST_species_count.txt")

         
